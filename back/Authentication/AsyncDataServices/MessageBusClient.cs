...
namespace Authentication.AsyncDataServices
{
    public class MessageBusClient : IMessageBusClient
    {
        ...
        public void PublishEntrepriseProfil(EntreprisePublishedDto entrepriseDto)
        {
            var message = JsonSerializer.Serialize(entrepriseDto);

            if (_connection.IsOpen)
            {
                SendMessage(message, "Entreprise");
            }
            else
            {
        ...
        public void PublishLivreurProfil(LivreurPublishedDto livreurDto)
        {
            var message = JsonSerializer.Serialize(livreurDto);

            if (_connection.IsOpen)
            {
                SendMessage(message, "Livreur");
            }
            else
            {
                throw new HttpRequestException("publish problem", null, HttpStatusCode.InternalServerError);
