...
export class AuthService {
  ...
  private readonly COOKIE_NAME_KEY = 'fullname';
  private readonly COOKIE_ROLE_KEY = 'role';
    ...
    this.loginService.login(user).subscribe({
      ...
      error: (error) => {
        ...
        if (error.error == "credentials") {
          ...
          this.spinner.show();
          this.loginService.getPartialUser(user.email).subscribe({
            next: (response) => {
              const expires = new Date();
              expires.setMinutes(expires.getMinutes() + 30);
              this.saveRegistrationInfos(response);
  ...
  saveRegistrationInfos(infos: any) {
    const expires = new Date();
    expires.setMinutes(expires.getMinutes() + 30);
    this.cookieService.set(this.COOKIE_NAME_KEY, infos.fullname, expires, '/', '', true, "Strict");
    this.cookieService.set(this.COOKIE_ROLE_KEY, infos.role, expires, '/', '', true, "Strict");
  }
  ...
  async getCurrentUserId(): Promise<string> {
    const email = this.getFullName();
    return await this.loginService.getCurrentUserIdByEmail(email);
  }
}
